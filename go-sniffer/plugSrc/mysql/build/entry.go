		...
		case packet := <-stm.packets:
			if packet.length != 0 {
				if packet.isClientFlow {
					stm.resolveClientPacket(packet.payload, packet.seq, packet.tmpNet, packet.tmpTransport)
				} else {
					stm.resolveServerPacket(packet.payload, packet.seq, packet.tmpNet, packet.tmpTransport)
				}
			}
		}
	}
...
func (stm *stream) resolveServerPacket(payload []byte, seq int, net, transport gopacket.Flow) {
...
func (stm *stream) resolveClientPacket(payload []byte, seq int, net, transport gopacket.Flow) {
